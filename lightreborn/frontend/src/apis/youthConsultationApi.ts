...
// 은둔 고립 청년 발굴 및 선정 절차에 있는 청년 데이터 조회 
export const getYouthConsultationPreSupport = async (
  page: number = 0,
  size: number = 7,
  sort: string = "personalInfo.name.ASC",
) => {
  const response = await api.get("/youth-consultation/isolated-youths/pre-support", {
    params: { page, size, sort: [sort]}
  });
  return response.data;
};

// 은둔 고립 청년 정보(이름, 연령, 점수 등)리스트를 반환하는 함수
export const getYouthConsultation = async () => {
  const response = await api.get("/youth-consultation/isolated-youths");
  return response.data;
};
