  ...
  export default function Sheet({
    ...
    const renderChart = () => {
      ...
      const chartData: BarChartItem[] = processedData.map(item => {
        const typeItem = item as Record<string, unknown>;
        return {
          name: typeItem[nameKey as string]?.toString() || '',
          value: Number(typeItem[valueKey as string]) || 0,
          color: colorKey && typeItem[colorKey as string] 
            ? (typeItem[colorKey as string] as string)  // 여기에 추가 타입 단언
            : (highlightItem && typeItem[nameKey as string] === highlightItem ? highlightColor : defaultColor)
        };
      });
